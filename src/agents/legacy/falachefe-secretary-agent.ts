...
import { userProfileTool } from '@/lib/crewai/user-profile-tool'
import { z } from 'zod'
  ...
  execute: async (args, runContext?: RunContext<{ userId: string }>): Promise<string> => {
    ...
    try {
      ...
      if (businessContext.goals?.length > 0) {
        response += `🎯 **Metas:**\n${businessContext.goals.map((goal: string) => `• ${goal}`).join('\n')}\n\n`
      }
      if (businessContext.priorities?.length > 0) {
        response += `⚡ **Prioridades:**\n${businessContext.priorities.map((priority: string) => `• ${priority}`).join('\n')}\n\n`
      }
      if (businessContext.mainChallenges?.length > 0) {
        response += `🔧 **Principais Desafios:**\n${businessContext.mainChallenges.map((challenge: string) => `• ${challenge}`).join('\n')}\n\n`
      }
