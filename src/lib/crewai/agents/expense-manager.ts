...
export class ExpenseManager {
  ...
  async addExpense(
    ...
    try {
      ...
      // Save to database
      const [expense] = await db.insert(financialData).values({
        type: 'expense',
  ...
  async addRevenue(
    ...
    try {
      ...
      // Save to database
      const [revenue] = await db.insert(financialData).values({
        type: 'revenue',
  ...
  async getFinancialData(
    ...
    try {
      ...
      const query = db.select().from(financialData)
        .where(and(...conditions))
        .orderBy(desc(financialData.date))
  ...
  async getTotalExpenses(userId: string): Promise<number> {
    try {
      const data = await db.select().from(financialData)
        .where(
          and(
            eq(financialData.userId, userId),
            eq(financialData.type, 'expense')
          )
        )
      ...
      const data = await db.select().from(financialData)
        .where(
          and(
            eq(financialData.userId, userId),
            eq(financialData.type, 'expense'),
            gte(financialData.date, startOfMonth)
          )
        )
