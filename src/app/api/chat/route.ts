...
async function processMessage(message: string, userId?: string, conversationId?: string): Promise<any> {
  console.log('🚀 Processing message with OpenAI Agents SDK and Memory System')
  
  try {
    const result = await falachefeSecretaryAgent.processMessage(message, userId, conversationId)
    console.log('✅ OpenAI Agents SDK with Memory Response generated successfully')
    return result
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`✅ Response generated by ${response.agentId} (confidence: ${response.confidence})`)
