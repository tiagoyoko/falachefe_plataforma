...
/**
 * Script de teste para verificar se as tools do agente estão funcionando corretamente
...
const getBusinessContextTool = tool({
  ...
  execute: async (args, runContext?: RunContext<{ userId: string }>): Promise<string> => {
    ...
    try {
      ...
      if (result.success && result.data) {
        return `**Contexto Empresarial:**\n- Metas: ${result.data.goals?.join(', ')}\n- Prioridades: ${result.data.priorities?.join(', ')}`
      } else {
...
class TestFalachefeSecretaryAgent {
  ...
  async processMessage(message: string, userId?: string, conversationId?: string): Promise<{
    ...
    try {
      ...
      return {
        response: result.finalOutput || 'Resposta não disponível',
...
async function testAgentToolsFixed() {
  console.log('🧪 Testando agente com tools corrigidas...\n')

  // Verificar se a API key está configurada
  if (!process.env.OPENAI_API_KEY) {
    console.error('❌ OPENAI_API_KEY não está configurada!')
    console.log('Por favor, configure a variável de ambiente OPENAI_API_KEY')
    process.exit(1)
  }
  ...
  console.log('\n🎯 Teste com tools corrigidas concluído!')
}
